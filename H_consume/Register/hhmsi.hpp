...
typedef THHashString THElementNameType;
...
class THHMSI
{
  ...
  public:
	...
	// for extension
	THHashAddResult AddS(const string AKey, THElementValueType AValue);
	THHashAddResult AddP(char* AKey, THElementValueType AValue);
	bool RemoveS(const string AKey);
	bool RemoveP(char* AKey);
	bool ExistsS(const string AKey);
	bool ExistsP(char* AKey);
	PHElement FindS(const string AKey);
	PHElement FindP(char* AKey);
	PHElement FindExS(const string AKey, PHHashFindResult fr);
	PHElement FindExP(char* AKey, PHHashFindResult fr);
	THElementValueTypeF GetValueS(const string AKey);
	THElementValueTypeF GetValueP(char* AKey);
	// ext end;
	int CacheSize();
	void Clear();
	THHMSI(uint Size = H_HASHTABLE_SIZE);
	~THHMSI();
};
