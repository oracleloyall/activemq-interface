...
PHElement THHMSI::AcquireBuffer()
{
	PHElement p = m_eDict;
...
PHElement THHMSI::GetFreeElement()
{
...
inline uint THHMSI::Hash(THElementNameTypeF Key)
{
	uint ret = 0;
	crc32(&ret, Key->buf, Key->len);
	return ret;
}

bool THHMSI::RotateSubTree(PHElementPath path, int bf, int p)
{
	int leftBF, rightBF;
...
PHElement THHMSI::LL(PHElement root)
{
...
PHElement THHMSI::LR(PHElement root)
{
 	PHElement rootNext = (*root).left;
...
PHElement THHMSI::RR(PHElement root)
{
...
PHElement THHMSI::RL(PHElement root)
{
	PHElement rootNext = (*root).right;
...
void THHMSI::ExchangeElement(PHElement root, PHElement new_)
{
	THElement e;
	// 1. prepare
	void* ep = &e.Using;
...
inline int THHMSI::Compare(PHElement e1, PHElement e2)
{
	#ifdef H_HASH_PRECHECK
	if (e1->KeyHash < e2->KeyHash)
		return -1;
	else if (e1->KeyHash > e2->KeyHash)
		return 1;
	else
	#endif
    return strcmp(e1->Key.buf, e2->Key.buf);
}

//#ifdef H_HASH_PRECHECK
inline int THHMSI::CompareEx(THElementNameTypeF Key, /*uint Code,*/ PHElement Element)
{
  /*if (Code < Element->KeyHash)
   return -1;
  else if (Code > Element->KeyHash)
   return 1;
  else*/
	return strcmp(Key->buf, Element->Key.buf);
}
//#endif

THHashAddResult THHMSI::Add(const THElementNameTypeF AKey, const THElementValueTypeF AValue)
{
  // 1, check full
...
bool THHMSI::Remove(const THElementNameTypeF AKey)
{
  //hc, idx: Cardinal;
  //node: ;
  //path: THElementPath;
...
inline void THHMSI::ReleaseNode(PHElement node)
{
  node->Using = false;
...
void THHMSI::RemoveNode(PHElementPath path, PHElement node, int p)
{
  PHElement t; int bf;
...
bool THHMSI::Exists(const THElementNameTypeF Key)
{
  return Find(Key);
}

PHElement THHMSI::Find(const THElementNameTypeF Key)
{
  uint hc; uint idx;
  int i;
  // 2. check direct
...
PHElement THHMSI::FindEx(const THElementNameTypeF Key, PHHashFindResult fr)
{
  int i;
  fr->Occupy = false;
  // 1. no data, pass
...
THElementValueTypeF THHMSI::GetValue(const THElementNameTypeF Key)
{
 // hc, idx: Cardinal;
 // node: ;
 // i: Integer;
...
PHElement THHMSI::FindNext(PHElement e)
{
  if (IsEmpty()) return NULL;
...
inline PHElement THHMSI::GetElementByIndex(uint Index)
{
  return m_eDict + Index;
}

void THHMSI::ReleaseBuffer(PHElement push)
{
...
inline void hds_HashString(const string AKey, PHElementNameType Key)
{
	Key->len = AKey.length();
	strcpy(Key->buf, AKey.c_str());
}

inline void hds_HashString(char* AKey, PHElementNameType Key)
{
	Key->len = strlen(AKey);
	strcpy(Key->buf, AKey);
}

THHashAddResult THHMSI::AddS(const string AKey, THElementValueType AValue)
{
	THElementNameType	Key;
	hds_HashString(AKey, &Key);
	return Add(&Key, AValue);
}

THHashAddResult THHMSI::AddP(char* AKey, THElementValueType AValue)
{
	  THElementNameType	Key;
	  hds_HashString(AKey, &Key);
	  return Add(&Key, AValue);
...
bool THHMSI::RemoveS(const string AKey)
{
	THElementNameType	Key;
	hds_HashString(AKey, &Key);
	return Remove(&Key);
...
bool THHMSI::ExistsS(const string AKey)
{
	THElementNameType	Key;
	hds_HashString(AKey, &Key);
	return Exists(&Key);
...
PHElement THHMSI::FindS(const string AKey)
{
	THElementNameType	Key;
	hds_HashString(AKey, &Key);
	return Find(&Key);
...
PHElement THHMSI::FindP(char* AKey)
{
	THElementNameType	Key;
	hds_HashString(AKey, &Key);
	return Find(&Key);
...
PHElement THHMSI::FindExS(const string AKey, PHHashFindResult fr)
{
	THElementNameType	Key;
	hds_HashString(AKey, &Key);
	return FindEx(&Key, fr);
}

PHElement THHMSI::FindExP(char* AKey, PHHashFindResult fr)
{
	THElementNameType Key;
	hds_HashString(AKey, &Key);
	return FindEx(&Key, fr);
...
THElementValueTypeF THHMSI::GetValueS(const string AKey)
{
	THElementNameType Key;
	hds_HashString(AKey, &Key);
	return GetValue(&Key);
...
THElementValueTypeF THHMSI::GetValueP(char* AKey)
{
	THElementNameType Key;
	hds_HashString(AKey, &Key);
	return GetValue(&Key);
